...
//    }
	...
	float maxx = MIN(AZScaleRect(frame, .5).size.width,AZScaleRect(frame, .5).size.height);
	image.size = NSMakeSize(maxx, maxx);
	image = [image tintedWithColor:RANDOMCOLOR];
...
- (void)drawKnob:(NSRect)rect
{
	...
	image.scalesWhenResized = YES;
	float maxx = MIN(AZScaleRect(knobRect, .5).size.width,AZScaleRect(knobRect, .5).size.height);
	image.size = NSMakeSize(maxx, maxx);
	image = [image tintedWithColor:RANDOMCOLOR];
